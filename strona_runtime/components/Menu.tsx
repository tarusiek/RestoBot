...
export default function Menu() {
  ...
  return (
    <section id="menu" className="py-24 md:py-32 bg-gradient-to-b from-accent/30 to-background border-t border-white/5">
      <div className="container mx-auto px-6 md:px-12">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {menuData[activeCategory].items.map((item, idx) => (
                <div key={idx} className="flex flex-col md:flex-row justify-between items-baseline gap-4 group p-4 -mx-4 rounded-lg hover:bg-white/[0.02] transition-colors duration-300">
                  <div className="flex-1">
                    <div className="flex items-center gap-4 mb-2">
                      <h3 className="text-xl font-heading text-white group-hover:text-primary transition-colors">{item.name}</h3>
                      <div className="hidden md:block flex-1 border-b border-white/10 border-dotted mt-4 opacity-50 group-hover:opacity-100 transition-opacity" />
                    </div>
                    <p className="text-muted-foreground font-light text-sm md:text-base leading-relaxed pr-8">{item.description}</p>
                  </div>
                  <div className="text-xl font-heading text-white whitespace-nowrap md:pl-4">
                    {item.price}
                  </div>
                </div>
              ))}
        ...
        <div className="mt-20 text-center">
          <a
            href="#"
            className="inline-flex items-center gap-3 px-8 py-4 bg-white/5 backdrop-blur-sm border border-white/20 text-white font-medium tracking-widest uppercase hover:bg-white/10 transition-all duration-300"
          >
